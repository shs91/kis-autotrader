...
> ⚠️ **상태가 `ready`가 아님 (검토대기).** 이 변경은 리스크 게이트(일일 손실 halt)를 **완화**하는 방향이므로 CLAUDE.md/BRIDGE_SPEC의 고위험 변경 규칙에 따라 자동 구현(auto-implement)을 보류한다. 사람이 수정안 (a)/(b)를 확정한 뒤 `상태: ready`로 전환할 것.
