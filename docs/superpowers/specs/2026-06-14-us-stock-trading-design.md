...
## 9. API 호출 예산 / 한도 안전성 (2026-06-16 검토 반영)

### 9.0 한·미 동시 운영 한도 안전성 — 결론: 안전 (이중 보호)
KIS REST 초당 한도는 **계좌(=앱키) 단위**(실전 20/초, 모의 2/초, 토큰발급 1/초)이며 **다른 앱키는 합산되지 않고 독립 버킷**이다(high). KIS 공식 정책에 **일일 호출 한도는 근거 없음**(medium) — 코드의 `daily_limit=50000`은 KIS 강제가 아닌 **자체 안전장치**. `EGW00201`은 초당 버스트 초과(일일 소진 아님, high).

따라서 본 설계(국내=모의 앱키 / 미국=실전 US 앱키, **분리 앱키**)는 **이중 안전**:
1. **다른 앱키 → KIS 초당 한도 독립**(국내 2~5/초와 미국 20/초가 별개 버킷).
2. **시간대 비중첩**(국내 08:25~16:00 KST vs 미국 22:30~06:00 KST DST) → 실제 동시 호출 자체가 없음.

> **미래 시나리오 B**(국내도 실전 전환 → 양 시장 한 실전 앱키 공유): 초당 20/초를 공유하나 시간대 비중첩이 방패. 경계시간(국내 결산 ↔ 미국 야간 준비)만 주의.

### 9.1 손봐야 할 멀티프로세스 함정 (기존 인프라에 미국 추가 시)
기존 코드에 이미 `RedisRateLimiter`/`HybridRateLimiter`(role 기반 quota)와 멀티프로세스(메인 엔진 + `ScreeningWorker` + `NewsCollectorWorker`)가 존재. 미국 추가 시:
- **Redis 일일 카운터 키 분리**: 현재 `kis_daily_count:{date}`에 앱키/시장 구분이 없어, 같은 Redis 공유 시 **다른 앱키인데도 일일 카운트 합산 → 미국이 국내 때문에 조기 차단**. → `kis_daily_count:{market}:{date}`로 분리.
- **시장별 독립 RateLimiter**: 다른 앱키는 KIS가 독립 처리하므로 limiter도 시장별 분리(`MarketProfile.rate_limit` 주입). 공유하면 불필요하게 묶임.
- **`per_second` 정정**: 모의 실제 한도는 **2/초**(코드 `virtual=5`). 미국 `real=20`은 무관하나 국내 모의 잠재 버스트 별도 확인.
- 토큰 발급(`_issue_token`)은 rate limiter 미통과·프로세스별 독립이므로 합산 무관.

### 9.2 초당 버스트(EGW00201) 방지 — 미국 사이클
미국은 EXCD 3개(NAS/NYS/AMS) 반복 + 종목당 시세 호출로 **초당 버스트** 압박이 큼(일일 아님):
- `_calculate_trading_interval`을 `MarketProfile`별 분리, **사이클 간격 하한**을 미국용 보수 설정.
- **유니버스 상한**: EXCD별 Top-N으로 제한(예: 거래소당 N → 총 3N).
- 미국 자체 `daily_limit`은 앱키별 독립 카운트(시장별 키 분리)로 관리 — KIS 강제 아님이므로 운영 모니터링하며 튜닝.
- 구현 시 **초당 소비 곡선 산정표** 작성(종목수 × 거래소수 × 사이클빈도 vs 20/초).
